...
exports.createDots = exports.initCanvas = exports.generateCircleData = exports.setCircleData = exports.drawCircle = exports.createSVG = undefined;
...
var dispatch = d3.dispatch('remove-circle');
var rand = function rand(modifier) {
	return Math.random() * modifier;
};

dispatch.on('remove-circle', function (canvas) {
...
var createSVG = exports.createSVG = function createSVG(id, width, height) {
	var svg = d3.select('#' + id).append('svg').attr('width', width).attr('height', height).attr('preserveAspectRatio', 'xMinYMin meet').attr('viewBox', '0 0 ' + width + ' ' + height);

	return svg;
};

var drawCircle = exports.drawCircle = function drawCircle(canvas, data) {
	canvas.append('circle').attr('cx', data.cx).attr('cy', data.cy).attr('r', 0).style('stroke-opacity', 1).style('stroke', data.color).style('fill', 'none').transition().duration(1000).ease(d3.easeQuadIn).attr('r', 100).style('stroke-opacity', 0.001).remove();
};

var setCircleData = exports.setCircleData = function setCircleData(canvas, circle, data, index) {
	d3.select(circle).attr('class', 'circle').attr('cx', function (d) {
		return data.cx;
	}).attr('cy', function (d) {
		return data.cy;
...
var generateCircleData = exports.generateCircleData = function generateCircleData(count) {
	return d3.range(count).map(function () {
		var cx = rand(innerWidth);
		var cy = rand(innerHeight);
		var r = rand(100);
		var op = rand(1);
		var color = d3.hsl(circleIndex = (circleIndex + 1) % 360, 1, 0.5);
...
var initCanvas = exports.initCanvas = function initCanvas(id) {
	var i = 0;
	...
	canvas.on(eventType, function () {
		...
		var color = d3.hsl(i = (i + 1) % 360, 1, 0.5);
...
var createDots = exports.createDots = function createDots(canvas) {
	var circles = canvas.selectAll('circle').data(generateCircleData(30)).enter().append('circle').each(function (d, i) {
...
exports.drawPercentageText = exports.getTspan = exports.arcTween = exports.drawChartArcs = exports.drawDonut = exports.dispatch = undefined;

var _d = __webpack_require__(33);

var d3 = _interopRequireWildcard(_d);

var _canvas = __webpack_require__(91);
...
var halfSize = size / 2;
var fontSize = '2.0rem';
var innerRadius = halfSize - 10;
var transitionType = d3.easeQuadIn;
var transitionsDelay = 250;
var transitionsDuration = 1000;
var arc = d3.arc().outerRadius(halfSize).innerRadius(innerRadius);
var pie = d3.pie().value(function (d) {
	return d.value;
});
var svg = (0, _canvas.createSVG)('skill-donut', size, size);

var dispatch = exports.dispatch = d3.dispatch('change-skill');

dispatch.on('change-skill', function (data) {
...
var drawDonut = exports.drawDonut = function drawDonut(data) {
	var group = svg.data([[data]]).append('g').attr('transform', 'translate(' + halfSize + ',' + halfSize + ')');

	drawChartArcs(group, data);
	drawPercentageText(group, data);
};

var drawChartArcs = exports.drawChartArcs = function drawChartArcs(svg, data) {
	svg.selectAll('g').data(pie).enter().append('path').attr('fill', data.color).each(function (d) {
		d.endAngle = 0;
	}).attr('d', arc).transition().duration(transitionsDuration).delay(transitionsDelay).ease(transitionType).call(arcTween, undefined);
};

var arcTween = exports.arcTween = function arcTween(transition) {
	transition.attrTween('d', function (d) {
		var min = 0;
		var max = 360 * (d.value / 100) * Math.PI / 180;
		var interpolate = d3.interpolate(min, max);
...
var getTspan = exports.getTspan = function getTspan(text, y) {
	return '<tspan x="0" dy="' + y + '">' + text + '</tspan>';
};

var drawPercentageText = exports.drawPercentageText = function drawPercentageText(svg, data) {
	svg.append('text').data([data]).attr('font-family', 'Anton').attr('font-size', '0rem').attr('fill', data.color).attr('text-anchor', 'middle').attr('y', '1rem').html(getTspan(data.text, -20) + getTspan(data.value + '%', 50)).transition().attr('font-size', fontSize).duration(transitionsDuration).delay(transitionsDelay).ease(transitionType);
};
...
var skills = exports.skills = [{ text: 'JavaScript', size: 90, value: 100 }, { text: 'React', size: 75, value: 75 }, { text: 'AngularJS', size: 80, value: 90 }, { text: 'Vue', size: 85, value: 90 }, { text: 'Backbone', size: 55, value: 65 }, { text: 'D3', size: 82, value: 95 }, { text: 'jQuery', size: 79, value: 100 }, { text: 'nodejs', size: 66, value: 85 }, { text: 'Gulp', size: 72, value: 100 }, { text: 'Grunt', size: 50, value: 95 }, { text: 'Webpack', size: 69, value: 90 }, { text: 'CSS / CSS3', size: 88, value: 100 }, { text: 'LESS', size: 60, value: 100 }, { text: 'SASS', size: 70, value: 100 }, { text: 'PostCSS', size: 77, value: 100 }, { text: 'HTML / HTML5', size: 99, value: 100 }, { text: 'SVG', size: 84, value: 95 }, { text: 'Ionic', size: 62, value: 90 }, { text: 'Bootstrap', size: 75, value: 100 }, { text: 'Foundation', size: 63, value: 95 }, { text: 'SVN', size: 57, value: 80 }, { text: 'GIT', size: 74, value: 95 }, { text: 'PHP', size: 80, value: 85 }, { text: 'WordPress', size: 89, value: 85 }, { text: 'Squarespace', size: 50, value: 80 }, { text: 'Shopify', size: 58, value: 75 }, { text: 'Photoshop', size: 61, value: 95 }, { text: 'Illustrator', size: 67, value: 70 }, { text: 'Sketch', size: 76, value: 85 }, { text: 'Affinity Designer', size: 81, value: 90 }, { text: 'Bash', size: 56, value: 70 }, { text: 'Meteor', size: 70, value: 70 }];
...
var _skillsList = __webpack_require__(175);

var _heroTexts = __webpack_require__(473);
...
(0, _skills.drawSkills)(_skillsList.skills);
(0, _canvas.createDots)(canvas);
(0, _slider.textRotate)('text', _heroTexts.texts);
(0, _donut.drawDonut)({ value: 100, color: 'black', text: 'Test' });
...
var textRotate = exports.textRotate = function textRotate(id, data) {
	var changeText = function changeText(index) {
		d3.select('#' + id).transition().duration(500).style('transform', 'scale(0)').transition().duration(500).text(data[index]).style('transform', 'scale(1)').on('end', function () {
			setTimeout(function () {
...
exports.drawSkills = exports.drawCloud = undefined;
...
var cloud = __webpack_require__(472);

var width = window.innerWidth / 3 * 2;
var height = 768;
var fill = [].concat(_toConsumableArray(d3.schemeCategory20), _toConsumableArray(d3.schemeCategory20));

var drawCloud = exports.drawCloud = function drawCloud(words) {
	var svg = (0, _canvas.createSVG)('skills-cloud', width, height);

	svg.append('g').attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')').selectAll('text').data(words).enter().append('text').style('font-family', 'Anton').style('font-size', function (d) {
		return d.size + 'px';
	}).attr('fill', function (d, i) {
		return fill[i];
	}).attr('text-anchor', 'middle').attr('transform', function (d) {
		return 'translate(' + [d.x, d.y] + ')rotate(' + d.rotate + ')';
	}).attr('data-score', function (d) {
		return d.score;
	}).text(function (d) {
		return d.text;
	}).on('click', function (d) {
		_donut.dispatch.call('change-skill', null, {
...
var drawSkills = exports.drawSkills = function drawSkills(skills) {
	cloud().size([width, height]).words(skills).rotate(function () {
		return ~~(Math.random() * 2) * 90;
	}).font('Anton').fontSize(function (d) {
		return d.size;
